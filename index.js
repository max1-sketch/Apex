...
function getWebsiteUrl() {
    ...
    const url = process.env.WEBSITE_URL || process.env.RENDER_EXTERNAL_URL || 'http://localhost:3000';
    return url.replace(/\/+$/, '');
}
...
<body>
  ...
  <script>
    document.getElementById('f').addEventListener('submit', async (e) => {
      ...
      const password = document.getElementById('pw').value;
      const res = await fetch('/api/login', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ password }) });
      window.location.reload(); // re-checks the maintenance gate either way
    });
  </script>
</body></html>`;
}

// Runs before auth on every public route. Staff who are already logged in pass straight
// through (so they can keep working/checking things); everyone else — including ticket-
// opener token links — sees the maintenance page instead.
...
app.get('/', maintenanceGate, requireAuth, (req, res) => res.send(renderTemplate(path.join(__dirname, 'views', 'index.html'))));
app.get('/transcript/:id', maintenanceGate, requireAuthOrTicketToken, (req, res) => res.send(renderTemplate(path.join(__dirname, 'views', 'transcript.html'))));
...
// Catch-all — without this, any unhandled error in a route just shows a blank
// "Internal Server Error" with nothing in the logs to explain why.
...
// CRITICAL: without a listener here, Node treats a Client 'error' event with zero listeners
// as fatal and kills the whole process. discord.js re-emits unhandled promise rejections from
// event handlers (e.g. a reply that fails because the interaction was already acknowledged)
// through exactly this event — this one listener is what stops a single failed interaction
// from taking down the entire bot.
client.on('error', (err) => {
...
client.on('interactionCreate', async (interaction) => {
    ...
    try {
        ...
        // -----------------------------------------------------------------
        // /config-site BUTTONS (pause/resume, maintenance mode)
        // -----------------------------------------------------------------
            ...
            if (customId.startsWith('open_ticket_')) {
                ...
                try {
                    ...
                    return interaction.reply({ content: `❌ Couldn't open the ticket form: ${err.message}`, ephemeral: true }).catch(fallbackErr => {
                        // If even the fallback reply fails (e.g. the interaction was already
                        // acknowledged by another running instance of this bot), just log it —
                        // an unhandled rejection here would crash the entire process, not just
                        // this one button click.
...
// Last-resort safety net — logs anything that somehow still slips past the guards above
// instead of letting an unrelated future bug silently take the whole bot offline.
